...
class TelegramNotifier:
    ...
    def _calc_today_pnl(self, trades):
        ...
        for t in trades:
            ...
            try:
                if datetime.fromisoformat(closed_at).date() != today:
                    continue
            except Exception:
                continue
            p = t.get("pnl", 0.0)
            ...
            if p > 0:
                ...
                losses += 1
        return round(pnl, 4), wins, losses, wins + losses
    ...
    def _cmd_help(self):
        self.send(
            ...
            "/htf off         — Disable HTF filter entirely\n"
            "/invalid         — List all invalid/blacklisted symbols\n"
            "/remove SYM/USDT — Remove symbol from invalid list\n"
        )
    ...
    def _cmd_htf(self, text: str):
        ...
        if len(parts) == 1:
            ...
            desc = {
                ...
                "off":    "No HTF filtering — all signals pass through",
            }
            self.send(
                ...
                f"{desc.get(mode,'')}\n\n"
                f"Change with:\n"
                f"/htf strict | /htf soft | /htf hard | /htf off"
            )
            return

        mode = parts[1].lower()
        if mode not in HTF_MODES:
            self.send(f"Unknown mode <b>{mode}</b>. Use: strict | soft | hard | off")
            return
    ...
    def _cmd_invalid(self):
        p = DATA / "invalid_symbols.json"
        if not p.exists():
            self.send("No invalid symbols on file.")
            return
        with open(p) as f:
            symbols = json.load(f)
        if not symbols:
            self.send("Invalid symbols list is empty.")
            return
        lines = "\n".join(f"  • {s}" for s in sorted(symbols))
        self.send(
            f"<b>Invalid Symbols ({len(symbols)})</b>\n\n"
            f"{lines}\n\n"
            f"Remove one with:\n<code>/remove SYMBOL/USDT</code>"
        )

    def _cmd_remove(self, text: str):
        parts = text.strip().split(None, 1)
        if len(parts) < 2:
            self.send("Usage: <code>/remove SYMBOL/USDT</code>")
            return
        ...
        if symbol not in symbols:
            self.send(f"<b>{symbol}</b> not in invalid list.")
            return
        symbols.remove(symbol)
        with open(p, "w") as f:
            json.dump(symbols, f)
        self.send(f"Removed <b>{symbol}</b> from invalid list. Bot will retry it next scan.")
