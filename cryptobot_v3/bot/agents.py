...
class AgentCoordinator:
    ...
    def _read_groq_gate(self) -> float:
        for name in ("config_futures.yaml", "config_spot.yaml"):
            p = BOT_ROOT / name
            if p.exists():
                try:
                    with open(p) as f:
                        return float(yaml.safe_load(f).get("ml", {}).get("groq_gate_min_conf", 0.42))
                except Exception:
                    pass
        return 0.42
    ...
    def analyze(self, symbol, df, ml_signal):
        ...
        groq_gate  = self._read_groq_gate()
        worth_groq = (buy_votes >= 2 or sell_votes >= 2) and ml_conf >= groq_gate and ml_action != "HOLD"
