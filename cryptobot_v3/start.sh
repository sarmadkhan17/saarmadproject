...
if [ -d "$BOT_DIR/dashboard" ]; then
    screen -S dashboard_v3 -dm /bin/bash -c "cd $BOT_DIR/dashboard && PORT=$PORT /root/cryptobot_v3/venv/bin/python3 server.py"
    sleep 2
fi

# Start bot in screen
screen -dmS "$SCREEN_NAME" bash -c "cd $BOT_DIR/bot && BOT_MODE=$MODE /root/cryptobot_v3/venv/bin/python3 launcher.py"
sleep 2
