...
HTF_MODES  = ("strict", "soft", "hard", "off")
...
def detailed_health():
    ...
    bots   = [l for l in lines if ("launcher.py" in l or "spot_bot.py" in l or "futures_bot.py" in l)
              and "grep" not in l and "SCREEN" not in l and "bash" not in l]
...
INVALID_PATH = DATA / "invalid_symbols.json"
PROTECTED    = {"BTC/USDT", "ETH/USDT"}

@app.route("/api/invalid_symbols", methods=["GET"])
...
@app.route("/api/invalid_symbols/<path:symbol>", methods=["DELETE"])
def remove_invalid_symbol(symbol):
    symbol = symbol.upper().replace("-", "/")
    if symbol in PROTECTED:
        return jsonify({"error": f"{symbol} is protected and cannot be removed"}), 400
    if not INVALID_PATH.exists():
        return jsonify({"error": "No invalid symbols file"}), 404
    with open(INVALID_PATH) as f:
        symbols = json.load(f)
    if symbol not in symbols:
        return jsonify({"error": f"{symbol} not in invalid list"}), 404
    symbols.remove(symbol)
    with open(INVALID_PATH, "w") as f:
        json.dump(symbols, f)
    return jsonify({"removed": symbol, "remaining": sorted(symbols)})
...
EDITABLE_KEYS = {
    "strategy": ["htf_filter_mode", "min_confidence", "min_votes"],
    "ml":       ["forward_bars", "min_move", "groq_gate_min_conf", "lstm_min_accuracy", "retrain_hours"],
    "risk":     ["max_open_trades", "max_portfolio_heat", "max_daily_loss_pct",
                 "stop_loss_atr_multiplier", "take_profit_pct", "max_position_pct"],
    "bot":      ["scan_interval_seconds"],
}
...
def get_config():
    cfg = _read_cfg(CFG_FUT) or _read_cfg(CFG_SPOT)
    out = {}
    for section, keys in EDITABLE_KEYS.items():
        out[section] = {k: cfg.get(section, {}).get(k) for k in keys}
    return jsonify(out)
...
def settings():
    ...
    response = make_response(send_from_directory("static", "settings.html"))
...
if __name__ == "__main__":
    ...
    if certfile.exists() and keyfile.exists():
        ...
        print(f"Dashboard v3: https://localhost:{port}")
        app.run(host="0.0.0.0", port=port, debug=False, ssl_context=ssl_context)
    else:
        print(f"Dashboard v3 (HTTP fallback — certs not found): http://localhost:{port}")
