...
class CoinScanner:
        ...
        if p.exists():
            ...
            if last:
                ...
                if self.last_scan.tzinfo is None:
                    self.last_scan = self.last_scan.replace(tzinfo=LOCAL_TZ)
            if self.top_coins:
    ...
    def needs_scan(self, current_atr_ratio: float = 1.0):
        ...
        if current_atr_ratio > 1.5:   # high volatility → scan twice as often
            interval = interval / 2
        return (datetime.now(LOCAL_TZ) - self.last_scan).total_seconds() >= interval
...
and float(t.get("quoteVolume", 0) or 0) >= self.min_vol
        ...
        self.last_scan = datetime.now(LOCAL_TZ)
        self._save()
