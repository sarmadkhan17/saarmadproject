...
class TrendFilter:
    def __init__(self, cfg: dict) -> None:
        self.cfg = cfg
        # Relative EMA_fast change over the slow tier's slope_lookback window
        # (NOT per-bar). 0.02 = "strongly trending" only when the slow tier has
        # moved >2% over the window; smaller drifts no longer veto counter-trend
        # entries wholesale. See config_futures.yaml trend_filter.strong_slope_pct.
        self.strong_slope_pct: float = float(cfg.get("strong_slope_pct", 0.02))
