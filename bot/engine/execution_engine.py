...
class ExecutionEngine:
    ...
    def _execute_entry_locked(self, decision, symbol: str, action: str, price: float,
        ...
        self.notifier.send_alert(
            f"{head_icon} <b>{side_word} OPENED</b> · <b>{symbol}</b>\n"
            f"  🎯 Profile: {profile_name}   📊 Conf: {decision.adjusted_conf:.0%}\n"
            f"  💵 Size: ${est_usdt:.2f} USDT   💲 Entry: ${fill_price:.4f}\n"
            f"  🌐 HTF: {decision.htf_bias} | Regime: {decision.hmm_regime}\n"
            f"  ⚙️ Mode: {self.mode.upper()}"
        )
        return trade
