...
# Default (connect, read) timeout. A connect+read tuple is enforced so a stuck
# read can never block the single-threaded scan loop indefinitely — the bare
# scalar timeout failed to fire on poisoned keep-alive connections to the demo
# exchange, wedging the bot. Keepalive + bounded retries recycle dead sockets.
_DEFAULT_TIMEOUT = (5, 15)

# Enable TCP keepalive on every new pooled connection so the kernel detects and
# tears down half-open connections instead of leaving them in CLOSE-WAIT.
...
class _TimeoutHTTPAdapter(HTTPAdapter):
    """HTTPAdapter that applies a default timeout and TCP keepalive."""
    ...
    def init_poolmanager(self, *args, **kwargs):
        kwargs["socket_options"] = _KEEPALIVE_OPTS
...
def _build_session(api_key: str) -> requests.Session:
    """Session with bounded retries, connection recycling, and keepalive."""
    session = requests.Session()
    session.headers.update({"X-MBX-APIKEY": api_key})
    retry = Retry(
        total=2, connect=2, read=2, backoff_factor=0.5,
        status_forcelist=(429, 500, 502, 503, 504),
        allowed_methods=None,  # retry all verbs (idempotent on this API)
    )
    adapter = _TimeoutHTTPAdapter(max_retries=retry, pool_maxsize=20)
...
class BinanceDemoClient:
    ...
    def fetch_order_book(self, symbol: str, limit: int = 20) -> dict:
        ...
        sym  = self._normalize_symbol(symbol)
        data = self._public_get("/depth", {"symbol": sym, "limit": min(limit, 1000)})
        return {
            "symbol": symbol,
